...
Provider = Literal["openai", "anthropic", "huggingface", "server", "local"]
...
@dataclass
class LLMSelection:
    """Represents the chosen LLM provider and model/config.
...
def load_selection(base: Path | None = None) -> Optional[LLMSelection]:
    ...
    try:
        ...
        if provider not in ("openai", "anthropic", "huggingface", "server", "local"):
            return None
        return LLMSelection(provider=provider, model=model, base_url=base_url, extra=extra)
    except Exception:
        return None
...
def require_env_vars(sel: LLMSelection) -> Tuple[bool, list[str]]:
    ...
    if sel.provider == "openai":
        ...
        if not ((sel.model and sel.model.strip()) or (model_from_env and str(model_from_env).strip())):
...
def env_values_for(sel: LLMSelection) -> Dict[str, str]:
    """Read relevant env vars for the provider. Returns a dict of key->value for present keys only.
    ...
    if sel.provider == "openai":
        ...
        # Prefer the first (canonical) key in output, even if value comes from an alias
...
def effective_model(sel: LLMSelection, override: Optional[str] = None, default_model: Optional[str] = None) -> Optional[str]:
...
def configuration_warning_if_unset(base: Path | None = None) -> Optional[str]:
    ...
    return (
        ...
        "(e.g., OPENAI_API_KEY, ANTHROPIC_API_KEY, CRPB_OPENAI_MODEL, CRPB_ANTHROPIC_MODEL, CRPB_HF_MODEL — tokens optional). "
